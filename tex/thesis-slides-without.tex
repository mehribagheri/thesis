...
%\node (in1) [test,on chain] {Is the input valid?};
\node (out1) [process] {$k = 2$, $q = n$\\ $U_1=\emptyset$, $l = 1$\\$L_1=1$, $x_1\leftarrow c_1$};
\node (out2) [process] {determine};
\node (out3) [process,left of=out2] {$k=j-1$};
%\draw[arrow] (out3.east) -- node[auto] |- (start.east);
